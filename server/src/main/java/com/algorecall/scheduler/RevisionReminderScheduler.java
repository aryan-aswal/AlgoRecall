...
public class RevisionReminderScheduler {
    ...
    public void autoSkipOverdueRevisions() {
        ...
        List<RevisionSchedule> overdueRevisions = revisionScheduleRepository
                .findByScheduledDateBeforeAndStatus(today, RevisionSchedule.Status.PENDING);
        for (RevisionSchedule rs : overdueRevisions) {
            rs.setStatus(RevisionSchedule.Status.SKIPPED);
            revisionScheduleRepository.save(rs);
            log.info("Auto-skipped overdue revision #{} for problem '{}' (was scheduled for {})",
                    rs.getId(), rs.getProblem().getTitle(), rs.getScheduledDate());
        }
    }
    ...
    public void createTodayReminders() {
        LocalDate today = LocalDate.now();
        List<RevisionSchedule> todaysRevisions = revisionScheduleRepository
                .findByScheduledDateAndStatusAndNotified(today, RevisionSchedule.Status.PENDING, false);
